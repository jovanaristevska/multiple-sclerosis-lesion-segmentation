...
class nnUNetBaseDatasetLongi(nnUNetBaseDataset):
    """
...
class nnUNetDatasetNumpyLongi(nnUNetBaseDatasetLongi):
...
class nnUNetDatasetBlosc2Longi(nnUNetBaseDatasetLongi):
    def __init__(self, folder: str, identifiers: List[str] = None,
                 folder_with_segs_from_previous_stage: str = None):
        super().__init__(folder, identifiers, folder_with_segs_from_previous_stage)
...
file_ending_dataset_mapping = {
    'npz': nnUNetDatasetNumpyLongi,
    'b2nd': nnUNetDatasetBlosc2Longi
}


def infer_dataset_class(folder: str) -> Union[Type[nnUNetDatasetBlosc2Longi], Type[nnUNetDatasetNumpyLongi]]:
