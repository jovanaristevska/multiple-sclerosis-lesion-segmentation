...
def create_lists_from_splitted_dataset_folder_and_patients_json(folder: str, file_ending: str, patients_json: dict, identifiers: List[str] = None, 
                                                                num_processes: int = 12) -> List[List[str]]:
    """
    uses patients_json to filter for scans with patient information
    """
    if identifiers is None:
        identifiers = get_identifiers_from_splitted_dataset_folder(folder, file_ending)
    files = subfiles(folder, suffix=file_ending, join=False, sort=True)

    # filter patient_json with identifiers
    filtered_patients_json = {k: [i for i in v if i in identifiers] for k, v in patients_json.items()}

    # for now must work without multiprocessing, but the current implementation is not optimal anyways, so that's it
    list_of_lists = []
    for k, v in filtered_patients_json.items():
        if len(v) == 0: continue
        patient_files = [create_paths_fn(folder, files, file_ending, f) for f in v]
        list_of_lists.extend([[c, p] for c, p in zip(patient_files, patient_files[:1] + patient_files[:-1])])
