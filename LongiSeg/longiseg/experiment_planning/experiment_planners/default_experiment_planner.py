...
from longiseg.experiment_planning.experiment_planners.network_topology import get_pool_and_conv_props
...
from longiseg.preprocessing.normalization.map_channel_name_to_normalization import get_normalization_scheme
from longiseg.preprocessing.resampling.default_resampling import resample_data_or_seg_to_shape, compute_new_shape
...
class ExperimentPlanner:
    def __init__(self, dataset_name_or_id: Union[str, int],
        ...
        self.raw_dataset_folder = join(LongiSeg_raw, self.dataset_name)
        preprocessed_folder = join(LongiSeg_preprocessed, self.dataset_name)
    ...
    def plan_experiment(self):
        ...
        shutil.copy(join(self.raw_dataset_folder, 'dataset.json'),
                    join(LongiSeg_preprocessed, self.dataset_name, 'dataset.json'))
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(LongiSeg_preprocessed, self.dataset_name, self.plans_identifier + '.json')
        ...
        maybe_mkdir_p(join(LongiSeg_preprocessed, self.dataset_name))
        save_json(plans, plans_file, sort_keys=False)
        print(f"Plans were saved to {join(LongiSeg_preprocessed, self.dataset_name, self.plans_identifier + '.json')}")
