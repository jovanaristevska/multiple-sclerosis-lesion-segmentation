...
def extract_fingerprint_longi_dataset(dataset_id: int,
...
def extract_fingerprints_longi(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
                         num_processes: int = default_num_processes, check_dataset_integrity: bool = False,
                         clean: bool = True, verbose: bool = True):
    """
    clean = False will not actually run this. This is just a switch for use with LongiSeg_plan_and_preprocess where
    we don't want to rerun fingerprint extraction every time.
    """
    fingerprint_extractor_class = recursive_find_python_class(join(longiseg.__path__[0], "experiment_planning"),
                                                              fingerprint_extractor_class_name,
                                                              current_module="longiseg.experiment_planning")
    for d in dataset_ids:
        extract_fingerprint_longi_dataset(d, fingerprint_extractor_class, num_processes, check_dataset_integrity, clean,
                                    verbose)


def plan_experiment_longi_dataset(dataset_id: int,
...
def plan_experiments_longi(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
                     gpu_memory_target_in_gb: float = None, preprocess_class_name: str = 'DefaultPreprocessor',
                     overwrite_target_spacing: Optional[Tuple[float, ...]] = None,
    ...
    if experiment_planner_class_name == 'ExperimentPlanner':
        print("\n############################\n"
              ...
              "\n############################\n")
    experiment_planner = recursive_find_python_class(join(longiseg.__path__[0], "experiment_planning"),
                                                     experiment_planner_class_name,
                                                     current_module="longiseg.experiment_planning")
    plans_identifier = None
    for d in dataset_ids:
        _, plans_identifier = plan_experiment_longi_dataset(d, experiment_planner, gpu_memory_target_in_gb,
                                                      preprocess_class_name,
                                                      overwrite_target_spacing, overwrite_plans_name)
    return plans_identifier


def preprocess_longi_dataset(dataset_id: int,
...
def preprocess_longi(dataset_ids: List[int],
               plans_identifier: str = 'nnUNetPlans',
               configurations: Union[Tuple[str], List[str]] = ('2d', '3d_fullres', '3d_lowres'),
               num_processes: Union[int, Tuple[int, ...], List[int]] = (8, 4, 8),
               verbose: bool = False):
    for d in dataset_ids:
        preprocess_longi_dataset(d, plans_identifier, configurations, num_processes, verbose)
